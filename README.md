...
관리자 폼은 `/admin/` 입니다. 로컬에서는 개발 서버와 함께 `npm run cms`를 켭니다. 배포 사이트에서 GitHub로 로그인하려면 Cloudflare에 `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`을 넣고, GitHub OAuth App의 callback을 `https://triple-e6i.pages.dev/api/callback`로 두면 됩니다. 숙소 입력 때 객실은 3인실만 넣습니다.
