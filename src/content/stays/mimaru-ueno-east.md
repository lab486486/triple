---
nameKo: MIMARU 도쿄 우에노 히가시
nameEn: MIMARU Tokyo Ueno East
city: tokyo
area: ueno
chain: mimaru
party_types: [couple_child, three_adults]
sleep_layout: apartment
capacity_official: 6
capacity_comfortable: 4
room_size_sqm: 38
beds: "싱글 4 또는 싱글 2+2층침대"
child_sharing: true
extra_bed: false
station: 우에노역
walk_min: 3
price_band: mid
rakuten_url: https://hotel.travel.rakuten.co.jp/hotelsearch/?f_keyword=MIMARU+Tokyo+Ueno+East
official_url: https://mimaruhotels.com/en/hotel/ueno-east/
address: 東京都台東区東上野4-26-3
verdict: 성인 3인은 4싱글 아파트, 아이는 2층침대 패밀리 타입.
not_for: 1인당 요금을 비즈니스호텔 수준으로 맞추고 싶은 일정.
rank: 2
featured: false
updated: 2026-09-15
rooms:
  - name: Apartment 4 Single Beds
    size_sqm: 38
    beds: 싱글 4
    capacity_official: 4
    capacity_comfortable: 3
    party_types: [three_adults]
    recommended: true
    note: 성인 3인에게 가장 정직합니다. 남는 싱글에 짐을 두면 됩니다.
  - name: Family Apartment
    size_sqm: 39
    beds: 싱글 2 + 2층침대 1
    capacity_official: 6
    capacity_comfortable: 3
    party_types: [couple_child]
    recommended: true
    note: 초등 자녀가 위층을 좋아하면 이 타입입니다.
---

JR 우에노역 도보 3분. 객실 56실, 38~40㎡ 아파트가 기본이고 연결 화실은 78㎡입니다. 공식 정원은 성인 4 + 아동 2(6세 이하)입니다. 3인이 쓰면 침대가 남습니다. 화실 아파트는 바닥에 자는 것을 허용하는 가족용이라, 성인 3인이 각자 매트리스를 원하면 4싱글을 고르세요.

키친·식기·전자레인지가 객실에 있고, 세탁기는 코인 세탁입니다. 우에노역에서 가깝기 때문에 스카이라이너 첫날 일정이 맞습니다. 포켓몬 룸은 아이 일정용이지 성인 3인용이 아닙니다.
